...
export class FileHelper {
    /**
     * getUrlIconName
     * https://github.com/OfficeDev/office-ui-fabric-react/blob/master/packages/file-type-icons/src/FileTypeIconMap.ts
     * @param extension string
     */
    public static getUrlIconName = (extension: string) => {
        const fileType = this.getFileTypeFromExtension(extension);
        return `https://spoprod-a.akamaihd.net/files/fabric/assets/item-types/48/${fileType}.svg`;
    };

    private static getFileTypeFromExtension(extension: string): string {
        const fileType: string = Object.keys(FileTypeIconMap).find((key) =>
            FileTypeIconMap[key].extensions && FileTypeIconMap[key].extensions.includes(extension)
        );
