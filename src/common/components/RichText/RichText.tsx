...
import * as strings from 'RichTextStrings';
...
import PicturePicker from '../Picture Picker/PicturePicker';

const TOOLBARPADDING = 28;
/**
...
export class RichText extends React.Component<IRichTextProps, IRichTextState> {
  ...
  public componentDidMount(): void {
    ...
    if (this.props.isEditMode) {
      const target = document.getElementsByClassName('ms-Modal-scrollableContent')[0] || document;
      target.addEventListener('click', this.handleClickOutside);
      target.addEventListener('focus', this.handleClickOutside);
  ...
  public componentWillUnmount(): void {
    ...
    if (this.props.isEditMode) {
      const target = document.getElementsByClassName('ms-Modal-scrollableContent')[0] || document;
      target.removeEventListener('click', this.handleClickOutside);
      target.removeEventListener('focus', this.handleClickOutside);
    }
  }
   ...
   * @param prevState
   */
  public componentDidUpdate(prevProps: IRichTextProps): void {
    const target = document.getElementsByClassName('ms-Modal-scrollableContent')[0] || document;
  ...
  * Renders the 'Insert Image' dialog
  ...
  (
    <Dialog hidden={this.state.hideImageDialog}
      ...
      <PicturePicker onChange={(newValue: string) => {
        this.setState({
          insertImageUrl: newValue
        });
  ...
  private onChangeHeading = (_event: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {
    const newHeadingValue = item.key === 0 ? '' : item.key.toString();
    this.applyFormat('header', newHeadingValue);
  };

  private onChangeAlign = (_event: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {
    const newAlignValue = item.key === 'left' ? false : item.key.toString();
    this.applyFormat('align', newAlignValue);
  };

  private onChangeList = (_event: React.FormEvent<HTMLDivElement>, item?: IDropdownOption): void => {
  ...
  private applyFormat(name: string, value: any): void { // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    setTimeout(() => {
      this.handleChangeSelection(quill.getSelection());
    }, 100);
  }
   ...
   */
  private handleChangeSelection = (range): void => {
    const quill = this.getEditor();
    try {
      if (quill) {
        ...
        if (this._propertyPaneRef && this.state.morePaneVisible) {
          this._propertyPaneRef.onChangeSelection(range);
        }
      }
    } catch (error) {
      // no-op;
  ...
  private handleClickOutside = (event): void => {
    const outside = !elementContains(this._wrapperRef, event.target);
