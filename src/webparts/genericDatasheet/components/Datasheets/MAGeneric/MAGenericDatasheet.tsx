...
import { IMAGenericRequirementDatasheetProps, IMAGenericRequirementDatasheetState } from './MAGenericDatasheet.types';
...
//import MARequirementBasic from '../../../genericForm/components/MA Requirement Basic/MARequirementBasic';
...
export default class MAGenericRequirementDatasheet extends React.Component<
    IMAGenericRequirementDatasheetProps,
    IMAGenericRequirementDatasheetState
> {
    ...
    constructor(props: Readonly<IMAGenericRequirementDatasheetProps>) {
        super(props);
        this.state = {
                    ...
                    if (defaultItem) {
                        ...
                        switch (item[Consts.FIELDS.REQUIREMENTRELATIONSHIP_TYPE_FIELDS.MA_REQUIREMENTRELATIONSHIPTYPE]) {
                            ...
                            case strings.FormMARequirementGenericTypeLegalRepresentative:
                                ...
                                defaultItem.datasheetType = DatasheetType.LegalSvcProvider;
                                break;
                            ...
                            case strings.FormMARequirementGenericTypeRegulator:
                                ...
                                defaultItem.datasheetType = DatasheetType.Authority;
                                break;
                            // Group 3: DP
                            case strings.FormMARequirementTypeDistributionPartner:
                                ...
                                defaultItem.datasheetType = DatasheetType.DistributionPartner;
                                break;
                            // Group 4: SNP
                            case strings.FormMARequirementTypeSNP:
                                ...
                                defaultItem.datasheetType = DatasheetType.SNP;
                                break;
                            ...
                            case strings.FormMARequirementGenericTypeTeleportPartner:
                                ...
                                defaultItem.datasheetType = DatasheetType.TeleportPartner;
                                break;
    ...
    public initItem = async () => {
        ...
        try {
            ...
            let reletedItems: IReletedItem[] = null;
            ...
            const maRequirementItem: IMARequirement = {
                ...
                ResponsibleParty: item[Consts.FIELDS.MAGENERIC.RESPONSIBLEPARTY],
    ...
    private renderDatasheetInfo = (): JSX.Element | null => {
        const leftColumns = ['Summary', 'Markets', 'ResponsibleParty', 'ResponsiblePartyItem', 'ResponsiblePartyContacts', 'EutelsatOwner', 'GeoLeo', 'ReletedItems'];
        const rightColumns = ['RequirementType', 'Synthesis', 'RagStatus', 'MaVertical', 'Snps', 'ApplicationDate', 'EffectiveDate', 'EstimatedDate', 'Comments'];
    ...
    private renderDatasheetInfoRow = (key: string) => {
        ...
        const labelKey = `MAGenericRequirementDataSheetInfoLabel${key}`;
        return (
    ...
    private renderDatasheetInfoValue = (item, fieldName) => {
        ...
        if (fieldName === 'ReletedItems') {
            ...
            return (
                <div className={styles.relatedItemsTable}>
                    {/* <div className={styles.relatedItemsHeader}>
                        <strong>Related Items:</strong>
                    </div> */}
                    <div className={styles.relatedItemsContent}>
                        {relatedItems.map((relatedItem, index) => (
                            <div key={index} className={styles.relatedItemsRow}>
                                <div className={styles.relatedItemsLabel}>
                                    {relatedItem.relationShip} :
                                </div>
                                <div className={styles.relatedItemsItems}>
                                    {renderObjectPills(
                                        relatedItem.reletedItem,
                                        (item) => (
                                            <span key={item.key} className={styles.termPill}>
                                                <span className={styles.termPillText}>
                                                    <a
                                                        title={item.text}
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            const datasheetType = encodeURIComponent(relatedItem?.datasheetType);
                                                            const url = `${Config.SITE.ROOT_WEB_URL}/${Consts.Pages.DATASHEET_URL}?config=${datasheetType}&itemId=${item.key}`;
                                                            UrlHelper.navigate(url, false);
                                                        }}
                                                    >
                                                        {item.text}
    ...
    public render(): React.ReactElement<IMAGenericRequirementDatasheetProps> {
        const {
            showEditFormDialog,
            showDeleteDialog,
            item,
                            ...
                            <div className={styles.datasheetHeader}>
                                <div className={styles.datasheetTypeContainer}>
                                    < h5 className={styles.datasheetType} >
                                        {strings.MAGenricRequirementDataSheetTitle}
                                    </h5 >
                                </div>
                                ...
                                <Pivot linkSize="large" >
                                    ...
                                    {this.hasAnyRole(UserRole.Owner, UserRole.FinanceContributor, UserRole.FinanceVisitor, UserRole.Admin) && <PivotItem headerText="Fees" itemIcon="Bank" >
                                        <FeeDatasheetView
                                            ...
                                            relatedItemType={FormType.MARequirementGeneric}
                                        />
                                    </PivotItem>}
                                </Pivot>
                            </div>
    ...
    private renderEditFormDialog = () => {
        ...
        const editFormControl = (<MARequirementGeneric pnpService={pnpService} userService={userService}
            itemId={itemId} callback={async () => { await this.closeEditForm(true); }}></MARequirementGeneric>);
        const editFormTitle = strings.MAGenricRequirementDataSheetEditFormTitle;
