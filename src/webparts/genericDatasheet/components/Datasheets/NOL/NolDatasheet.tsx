...
import { INolDatasheetProps, INolDatasheetState } from './Nol.types';
...
import { IContact, IContentType, IFEE, IMarket, IItemOption, IMARequirement, ISNP } from '../../../../../common/models/IBusiness';
...
import { FeeChargedBy, FeeType, MARequirementType, MARequirementResponsibleParty, UserRole } from '../../../../../common/models/Enums';
...
export default class NolDatasheet extends React.Component<
    INolDatasheetProps,
    INolDatasheetState
> {
    ...
    constructor(props: Readonly<INolDatasheetProps>) {
        super(props);
        this.state = {
        ...
        this.config = Datasheet.config.find((f) => f.type.toLowerCase() === DatasheetType.MARequirementNol);
    }
    ...
    public initItem = async () => {
        ...
        const list = Datasheet.config.find((f) => f.type.toLowerCase() === DatasheetType.MARequirementNol).list;
        ...
        try {
            const selectedFields: string[] = [
                ...
                Consts.FIELDS.MA_REQUIREMENT_NOL.COMMENTS,
            ...
            const maRequirementItem: IMARequirement = {
                ...
                Comments: item[Consts.FIELDS.MA_REQUIREMENT_NOL.COMMENTS],
    ...
    private renderDatasheetInfo = (): JSX.Element | null => {
        const leftColumns = ['Summary', 'Markets', 'ResponsibleParty', 'ResponsiblePartyItem',
            'ResponsiblePartyContacts', 'EutelsatOwner', 'RequestedPartyType', 'RequestedPartyItem', 'RequestedPartyContacts', 'RagStatus', 'Synthesis'];
        const rightColumns = ['RequirementType', 'MaVertical', 'Snps', 'GeoLeo', 'Letter1SentDate', 'Letter2SentDate', 'Letter1Milestone',
            'Letter2Milestone', 'ResponseDate', 'ResponseSummary', 'Comments'];
    ...
    private renderDatasheetInfoRow = (key: string) => {
        ...
        const labelKey = `NolDataSheetInfoLabel${key}`;
        return (
    ...
    private renderDatasheetInfoValue = (item, fieldName) => {
        ...
        //const fieldName = column.fieldName as keyof INol | undefined;
        const value = fieldName ? item[fieldName] : undefined;
        ...
        if (fieldName === 'Comments' || fieldName === 'Address' || fieldName === 'Summary' || fieldName === 'ResponseSummary') {
    ...
    public render(): React.ReactElement<INolDatasheetProps> {
        const {
            showEditFormDialog,
            showDeleteDialog,
            item,
                                ...
                                <Pivot linkSize="large" >
                                    ...
                                    {this.hasAnyRole(UserRole.FinanceContributor, UserRole.FinanceVisitor, UserRole.Owner, UserRole.Admin) && <PivotItem headerText="Fees" itemIcon="Bank" >
                                        <FeeDatasheetView
                                            ...
                                            relatedItemType={FormType.MARequirementNol}
                                        />
                                    </PivotItem>}
                                </Pivot>
                            </div>
    ...
    private renderEditFormDialog = () => {
        ...
        const editFormControl = (<Nol pnpService={pnpService} userService={userService}
            itemId={itemId} callback={async () => { await this.closeEditForm(true); }}></Nol>);
        const editFormTitle = strings.NolDataSheetEditFormTitle;
