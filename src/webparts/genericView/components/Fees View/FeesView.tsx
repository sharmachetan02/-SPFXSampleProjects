...
import { IFEEViewState, IFEEViewProps, IItemOption } from './FeesView.type';
...
import { IContentType, IFEE } from '../../../../common/models/IBusiness';
...
import { ContactType, FeeType, UserRole } from '../../../../common/models/Enums';
...
export default class FeeView extends React.Component<
    IFEEViewProps,
    IFEEViewState
> {
    private config: IViewConfig;
    private allViewItems: IFEE[] = [];
    ...
    constructor(props: Readonly<IFEEViewProps>) {
        super(props);
        this.state = {
            isViewReady: false,
            columns: [],
        ...
        this.config = View.config.find((f) => f.type.toLowerCase() === ViewType.FEE);
    }
    ...
    public async componentDidUpdate(prev: IFEEViewProps) {
        if (
    ...
    private initColumns(): IColumn[] {
        const allColumns = [{
            key: 'Name',
            name: strings.FEEViewNameColumnLabel,
            fieldName: 'Title',
            minWidth: 300,
            maxWidth: 300,
            isResizable: true,
        ...
        {
            key: 'Summary',
            name: strings.FEEViewSummaryColumnLabel,
            fieldName: 'Summary',
            minWidth: 90,
            maxWidth: 90,
            isResizable: true,
        ...
        {
            key: 'ContentType',
            name: strings.FEEViewContentTypeColumnLabel,
        ...
        {
            key: 'Item',
            name: strings.FEEViewItemColumnLabel,
            fieldName: 'Item',
            minWidth: 180,
            maxWidth: 180,
            isResizable: true,
        ...
        {
            key: 'ChargeByItem',
            name: strings.FEEViewChargeByItemColumnLabel,
            fieldName: 'ChargeByItem',
            minWidth: 180,
            maxWidth: 180,
            isResizable: true,
            onColumnClick: this.onColumnClick
        ...
        {
            key: 'CostTotal',
            name: strings.FEEViewCostTotalColumnLabel,
            fieldName: 'CostTotal',
            minWidth: 90,
            maxWidth: 90,
            isResizable: true,
        ...
        {
            key: 'Status',
            name: strings.FEEViewStatusColumnLabel,
            fieldName: 'Status',
            minWidth: 90,
            maxWidth: 90,
            isResizable: true,
        ...
        {
            key: 'DueDate',
            name: strings.FEEViewDueDateColumnLabel,
            fieldName: 'DueDate',
            minWidth: 120,
            maxWidth: 120,
            isResizable: true,
    ...
    private initChargeByItemChoices = async (): Promise<IItemOption[]> => {
        const itemChoices: IItemOption[] = [];
        this.ChargeByItemChoices.forEach((item) => {
            if (!itemChoices.find((i) => i.key === item.key && i.type === item.type)) {
                itemChoices.push(item);
            }
        });
    ...
    private initFilters = (): IFilter[] => {
        ...
        const searchFilter: IFilter = {
            name: 'Search',
            displayName: strings.FEEViewTitleFilterLabel,
            placeHolder: strings.FEEViewTitleFilterPlaceHolder,
            fields: ['Title', 'ContentType', 'Item', 'CostTotal', 'Summary', 'Status', 'ChargeByItem'],
            selectedValues: [],
            values: [],
        ...
        const ContentTypeFilter: IFilter = {
            name: 'ContentType',
            displayName: strings.FEEViewContentTypeFilterLabel,
            placeHolder: strings.FEEViewContentTypeFilterPlaceHolder,
            fields: ['ContentType'],
            selectedValues: [],
            values: [],
        ...
        const itemFilter: IFilter = {
            name: 'Item',
            displayName: strings.FEEViewItemFilterLabel,
            placeHolder: strings.FEEViewItemFilterPlaceHolder,
            fields: ['Item'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        const ChargeByItemFilter: IFilter = {
            name: 'ChargeByItem',
            displayName: strings.FEEViewChargeByItemFilterLabel,
            placeHolder: strings.FEEViewChargeByItemFilterPlaceHolder,
            fields: ['ChargeByItem'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        ...
        const StatusTypeFilter: IFilter = {
            name: 'Status',
            displayName: strings.FEEViewStatusFilterLabel,
            placeHolder: strings.FEEViewStatusFilterPlaceHolder,
            fields: ['Status'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        allFilters = [searchFilter, ContentTypeFilter, itemFilter, ChargeByItemFilter, StatusTypeFilter];
    ...
    private init = async () => {
        ...
        if (this.props.viewItems) {
            ...
            this.props.viewItems.forEach((itm) => {
                if (itm.ChargeByItem) {
                    this.ChargeByItemChoices.push({
                        id: itm.ChargeByItem.id,
                        key: itm.ChargeByItem.key,
                        text: itm.ChargeByItem.text,
                        type: itm.ChargeByItem.type
                    });
                }
            });
        } else {
        ...
        let ChargeByItemChoices: IItemOption[] = [];
        ...
        if (this.isFilterEnabled('ChargeByItem')) {
            promises.push(this.initChargeByItemChoices().then((opts) => { ChargeByItemChoices = opts as IItemOption[]; }));
        }
        ...
        filters = filters.map((f) => {
            ...
            if (f.name === 'ChargeByItem') return { ...f, values: ChargeByItemChoices };
    ...
    private sort = (items) => {
        ...
        const key = sort.Property as keyof IFEE;
    ...
    private renderItemColumn = (item, index: number, column: IColumn) => {
        ...
        const fieldName = column.fieldName as keyof IFEE | undefined;
                ...
                <span key={itemValue.key} className={styles.termPill}>
                    <span className={styles.termPillText}>
                        <a data-interception="on"
                            rel="noreferrer" title={String(itemValue?.text)}
                            onClick={(e) => {
                                e.preventDefault();
                                const url = `${Config.SITE.ROOT_WEB_URL}/${Consts.Pages.DATASHEET_URL}?config=${viewType}&itemId=${itemValue.id}`;
                                UrlHelper.navigate(url, false);
        ...
        if (fieldName === 'CostTotal') {
            return (
                <div className={styles.tooltipHostContainer} title="">
                    <span className={styles.tooltipHostText}> {value || 0}</span>
                </div>
            );
        }
    ...
    private onChangeChargeByItem = (options: IItemOption[]) => {
        let { filters } = this.state;
        const itemFilter = filters.find((f) => f.name === 'ChargeByItem');
        itemFilter.selectedValues = options.length > 1 ? [options[options.length - 1]] : options;
        filters = filters.map(((f) => ((f.name === itemFilter.name) ? itemFilter : f)));
        this.setState({ filters, pageIndex: 0 });
    };
    ...
    public render(): React.ReactElement<IFEEViewProps> {
        ...
        const itemFilter = filters.find((f) => f.name === 'Item');
        const ChargeByItemFilter = filters.find((f) => f.name === 'ChargeByItem');
        const searchFilter = filters.find((f) => f.name === 'Search');
                                ...
                                <>
                                    ...
                                    >
                                        ...
                                        <div className={styles.containerPaneldiv}>
                                                ...
                                                <>
                                                    ...
                                                    <div className={styles.panelChildDiv}>
                                                        <MultiselectWrapper
                                                            ...
                                                            data={contentTypeFilter?.values as IItemOption[]}
                                            ...
                                            {/* ChargeByItem */}
                                            {
                                                this.isFilterEnabled('ChargeByItem') &&
                                                <>
                                                    <div >
                                                        <Label className={styles.filterLabel}>{ChargeByItemFilter.displayName}</Label>
                                                    </div>
                                                    <div className={styles.panelChildDiv}>
                                                        <MultiselectWrapper
                                                            placeholder={ChargeByItemFilter.placeHolder}
                                                            data={ChargeByItemFilter?.values as IDropdownOption[]}
                                                            dataKey={(item: IDropdownOption) => item.key}
                                                            textField={(item: IDropdownOption) => item.text}
                                                            value={ChargeByItemFilter?.selectedValues}
                                                            showSelectedItemsInList
                                                            showPlaceholderWithValues
                                                            renderTagValue={() => null}
                                                            filter="contains"
