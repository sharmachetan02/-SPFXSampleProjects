...
import { ITPViewProps, ITPViewState } from './TPView.types';
...
import { IMarket, ITP } from '../../../../common/models/IBusiness';
...
export default class TPView extends React.Component<
    ITPViewProps,
    ITPViewState
> {
    private config: IViewConfig;
    private allViewItems: ITP[] = [];
    constructor(props: Readonly<ITPViewProps>) {
        super(props);
        this.state = {
            isViewReady: false,
            columns: [],
        ...
        this.config = View.config.find((f) => f.type.toLowerCase() === ViewType.TeleportPartner);
    }
    ...
    public async componentDidUpdate(prev: ITPViewProps) {
        if (
    ...
    private initColumns(): IColumn[] {
        const allColumns = [{
            key: 'Title',
            name: strings.TPViewTitleColumnLabel,
            fieldName: 'Title',
            minWidth: 360,
            maxWidth: 360,
            isResizable: true,
        ...
        {
            key: 'Market',
            name: strings.TPViewMarketColumnLabel,
            fieldName: 'Markets',
            minWidth: 260,
            maxWidth: 260,
            isResizable: true
        ...
        {
            key: 'Summary',
            name: strings.TPViewSummaryColumnLabel,
            fieldName: 'Summary',
            minWidth: 120,
            maxWidth: 120,
            isResizable: true
        ...
        {
            key: 'Status',
            name: strings.TPViewStatusColumnLabel,
            fieldName: 'Status',
            minWidth: 120,
            maxWidth: 120,
            isResizable: true,
    ...
    public initMarketChoices = async (): Promise<IDropdownOption[]> => {
        const {
            pnpService
        } = this.props;
    ...
    public initViewItems = async () => {
        ...
        const items = await this.props.pnpService.getListItems(listUrl).
            ...
            expand(Consts.FIELDS.TELEPORT_PARTNER.MARKETS, Consts.FIELDS.TELEPORT_PARTNER.OWNER).
    ...
    private initFilters = (): IFilter[] => {
        ...
        const searchFilter: IFilter = {
            ...
            fields: ['Title', 'Markets', 'Status', 'Summary'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.Or,
        };
        const marketFilter: IFilter = {
            name: 'Market',
            displayName: strings.TPViewMarketFilterLabel,
            placeHolder: strings.TPViewMarketFilterPlaceHolder,
            fields: ['Markets'],
            selectedValues: [],
            values: [],
        ...
        const statusFilter: IFilter = {
            name: 'Status',
            displayName: strings.TPViewStatusFilterLabel,
            placeHolder: strings.TPViewStatusFilterPlaceHolder,
            fields: ['Status'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        allFilters = [searchFilter, marketFilter, statusFilter];
    ...
    private init = async () => {
        ...
        if (this.isFilterEnabled('Market')) {
            promises.push(this.initMarketChoices().then((opts) => { contries = opts as IDropdownOption[]; }));
        }
        ...
        filters = filters.map((f) => {
            if (f.name === 'Market') return { ...f, values: contries };
                ...
                <span className={styles.termPill}>
                  <span className={styles.termPillText}>
                    <a  data-interception="on"
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const url = `${Config.SITE.ROOT_WEB_URL}/${Consts.Pages.DATASHEET_URL}?config=${encodeURIComponent(ViewType.Market)}&itemId=${item.Id}`;
    ...
    private onChangeMarkets = (options: IDropdownOption[]) => {
        let { filters } = this.state;
        const marketFilter = filters.find((f) => f.name === 'Market');
        marketFilter.selectedValues = options;
        filters = filters.map(((f) => ((f.name === marketFilter.name) ? marketFilter : f)));
    ...
    public render(): React.ReactElement<ITPViewProps> {
                                ...
                                <>
                                    ...
                                    >
                                        <div className={styles.containerPaneldiv}>
                                            ...
                                            {/* Market */}
                                            {
                                                this.isFilterEnabled('Market') &&
                                                <>
                                                    <div >
                                                        <Label className={styles.filterLabel}>{marketFilter.displayName}</Label>
                                                    </div>

                                                    <div className={styles.panelChildDiv}>
                                                        <MultiselectWrapper
                                                            placeholder={marketFilter.placeHolder}
                                                            data={marketFilter?.values as IDropdownOption[]}
                                                            dataKey={(item: IDropdownOption) => item.key}
                                                            textField={(item: IDropdownOption) => item.text}
                                                            value={marketFilter?.selectedValues}
