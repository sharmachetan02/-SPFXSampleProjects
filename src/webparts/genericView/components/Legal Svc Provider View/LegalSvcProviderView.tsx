...
import { ILegalSvcProviderViewProps, ILegalSvcProviderViewState } from './LegalSvcProviderView.types';
...
import { IContentType, ICountry, ILegalSvcProvider } from '../../../../common/models/IBusiness';
...
export default class LegalSvcProviderView extends React.Component<
    ILegalSvcProviderViewProps,
    ILegalSvcProviderViewState
> {
    private config: IViewConfig;
    private allViewItems: ILegalSvcProvider[] = [];
    ...
    constructor(props: Readonly<ILegalSvcProviderViewProps>) {
        super(props);
        this.state = {
            isViewReady: false,
            columns: [],
        ...
        this.config = View.config.find((f) => f.type.toLowerCase() === ViewType.LegalSvcProvider);
    }
    ...
    public async componentDidUpdate(prev: ILegalSvcProviderViewProps) {
        if (
    ...
    private initColumns(): IColumn[] {
        const allColumns = [
            {
                key: 'Title',
                name: strings.LegalSvcProviderViewTitleColumnLabel,
                fieldName: 'Title',
                minWidth: 360,
                isResizable: true,
            ...
            {
                key: 'Country',
                name: strings.LegalSvcProviderViewCountryColumnLabel,
                fieldName: 'Countries',
                minWidth: 300,
                isResizable: true,
            },
            {
                key: 'ContentType',
                name: strings.LegalSvcProviderViewContentTypeColumnLabel,
            ...
            {
                key: 'Summary',
                name: strings.LegalSvcProviderViewSummaryColumnLabel,
                fieldName: 'Summary',
                minWidth: 120,
                isResizable: true
            },
            {
                key: 'Status',
                name: strings.LegalSvcProviderViewStatusColumnLabel,
                fieldName: 'Status',
                minWidth: 120,
                isResizable: true,
                onColumnClick: this.onColumnClick
    ...
    private initContentTypeChoices = async (): Promise<IDropdownOption[]> => (this.typeChoices);

    public initCountryChoices = async (): Promise<IDropdownOption[]> => {
        const {
            pnpService
        } = this.props;
    ...
    private initFilters = (): IFilter[] => {
        ...
        const searchFilter: IFilter = {
            ...
            fields: ['Title', 'ContentType', 'Countries', 'Status', 'Summary'],
            selectedValues: [],
            values: [],
        ...
        const contentTypeFilter: IFilter = {
            name: 'ContentType',
            displayName: strings.LegalSvcProviderViewContentTypeFilterLabel,
            placeHolder: strings.LegalSvcProviderViewContentTypeFilterPlaceHolder,
            fields: ['ContentType'],
            selectedValues: [],
            values: [],
        ...
        const countryFilter: IFilter = {
            name: 'Country',
            displayName: strings.LegalSvcProviderViewCountryFilterLabel,
            placeHolder: strings.LegalSvcProviderViewCountryFilterPlaceHolder,
            fields: ['Countries'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        ...
        const statusFilter: IFilter = {
            name: 'Status',
            displayName: strings.LegalSvcProviderViewStatusFilterLabel,
            placeHolder: strings.LegalSvcProviderViewStatusFilterPlaceHolder,
            fields: ['Status'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        allFilters = [searchFilter, contentTypeFilter, countryFilter, statusFilter];
    ...
    private getTypeChoiceTextByKey = (key: string | number): string | undefined => {
        // Ensure key is a string for indexOf
    ...
    private init = async () => {
        ...
        if (this.isFilterEnabled('Country')) {
            promises.push(this.initCountryChoices().then((opts) => { contries = opts as IDropdownOption[]; }));
        }
        ...
        filters = filters.map((f) => {
            ...
            if (f.name === 'Country') return { ...f, values: contries };
    ...
    public applyFilters = (): ILegalSvcProvider[] => {
        const { filters } = this.state;
                ...
                filter.fields.some((field) => {
                    ...
                    const assertions = selected.map((f) => {
                        ...
                        if (filter.name === 'Country') {
                            return (item[field] || []).some((c) => String(c.Id) === String((f as IDropdownOption).key));
                        }
    ...
    private sort = (items) => {
        ...
        const key = sort.Property as keyof ILegalSvcProvider;
    ...
    private renderItemColumn = (item, index: number, column: IColumn) => {
        ...
        const fieldName = column.fieldName as keyof ILegalSvcProvider | undefined;
                ...
                <span className={styles.termPill}>
                  <span className={styles.termPillText}>
                    <a  data-interception="on"
                      rel="noreferrer" title={item.Title}
                      onClick={(e) => {
                        e.preventDefault();
                        const url = `${Config.SITE.ROOT_WEB_URL}/${Consts.Pages.DATASHEET_URL}?config=${encodeURIComponent(ViewType.Country)}&itemId=${item.Id}`;
    ...
    public render(): React.ReactElement<ILegalSvcProviderViewProps> {
