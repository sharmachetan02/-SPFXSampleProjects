...
import { ISNPViewState, ISNPViewProps } from './SNPView.type';
...
import { IContentType, ICountry, ISNP, ITP } from '../../../../common/models/IBusiness';
...
import { SNPType, UserRole } from '../../../../common/models/Enums';
...
> {
    ...
    constructor(props: Readonly<ISNPViewProps>) {
        super(props);
        this.state = {
            isViewReady: false,
            columns: [],
        ...
        this.config = View.config.find((f) => f.type.toLowerCase() === ViewType.SNP);
    }
    ...
    public async componentDidUpdate(prev: ISNPViewProps) {
        if (
    ...
    private initColumns(): IColumn[] {
        const allColumns = [{
            key: 'Title',
            name: strings.SNPViewTitleColumnLabel,
            fieldName: 'Title',
            minWidth: 360,
            maxWidth: 360,
            isResizable: true,
        ...
        {
            key: 'Country',
            name: strings.SNPViewCountryColumnLabel,
            fieldName: 'Country',
            minWidth: 150,
            maxWidth: 150,
            isResizable: true,
        ...
        {
            key: 'City',
            name: strings.SNPViewCityColumnLabel,
            fieldName: 'City',
            minWidth: 150,
            maxWidth: 150,
            isResizable: true,
        ...
        {
            key: 'ContentType',
            name: strings.SNPViewContentTypeColumnLabel,
        ...
        {
            key: 'TeleportPartner',
            name: strings.SNPViewTeleportPartnerColumnLabel,
            fieldName: 'TeleportPartner',
            minWidth: 150,
            maxWidth: 150,
            isResizable: true,
            onColumnClick: this.onColumnClick
        ...
        {
            key: 'Status',
            name: strings.SNPViewStatusColumnLabel,
            fieldName: 'Status',
            minWidth: 120,
            maxWidth: 120,
            isResizable: true,
    ...
    private initCountriesChoices = async () => {
        ...
        const list = View.config.find((f) => f.type.toLowerCase() === ViewType.Country).list;
    ...
    public initTeleportPartnerChoices = async () => {
        ...
        const listUrl = UrlHelper.getListWebRelativeUrl(pnpService.getUrl(), Consts.LISTS.TELEPORT_PARTNER_URL);
        const selectedFields: string[] = [
            Consts.FIELDS.COMMON.ID,
            Consts.FIELDS.COMMON.TITLE
        ];
    ...
    private initFilters = (): IFilter[] => {
        ...
        const searchFilter: IFilter = {
            ...
            fields: ['Title', 'Country', 'ContentType', 'City', 'TeleportPartner', 'Status'],
            selectedValues: [],
            values: [],
        ...
        const countryFilter: IFilter = {
            name: 'Country',
            displayName: strings.SNPViewCountryFilterLabel,
            placeHolder: strings.SNPViewCountryFilterPlaceHolder,
            fields: ['Country'],
            selectedValues: [],
            values: [],
        ...
        const ContentTypeFilter: IFilter = {
            name: 'ContentType',
            displayName: strings.SNPViewContentTypeFilterLabel,
            placeHolder: strings.SNPViewContentTypeFilterPlaceHolder,
            fields: ['ContentType'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        const TeleportPartnerTypeFilter: IFilter = {
            name: 'TeleportPartner',
            displayName: strings.SNPViewTeleportPartnerFilterLabel,
            placeHolder: strings.SNPViewTeleportPartnerFilterPlaceHolder,
            fields: ['TeleportPartner'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        const StatusTypeFilter: IFilter = {
            name: 'Status',
            displayName: strings.SNPViewStatusFilterLabel,
            placeHolder: strings.SNPViewStatusFilterPlaceHolder,
            fields: ['Status'],
            selectedValues: [],
            values: [],
            operatorBetweenValues: LogicalOperator.And,
        };
        allFilters = [searchFilter, ContentTypeFilter, countryFilter, TeleportPartnerTypeFilter, StatusTypeFilter];
    ...
    private init = async () => {
        ...
        let teleportPartner: IDropdownOption[] = [];
        let status: IDropdownOption[] = [];
        const promises: Promise<void>[] = [];

        if (this.isFilterEnabled('Country')) {
        ...
        if (this.isFilterEnabled('TeleportPartner')) {
            promises.push(this.initTeleportPartnerChoices().then((opts) => { teleportPartner = opts as IDropdownOption[]; }));
        }
        ...
        filters = filters.map((f) => {
            if (f.name === 'Country') return { ...f, values: country };
            if (f.name === 'ContentType') return { ...f, values: contentType };
            if (f.name === 'TeleportPartner') return { ...f, values: teleportPartner };
                    ...
                    const assertions = selected.map((f) => {
                        if (filter.name === 'Search') {
                            ...
                            if (field === 'ContentType') {
                                ...
                                const hay = UtilHelper.toNormalForm((item[field] as ICountry)?.Title?.toString()?.toLowerCase());
                                return hay?.includes(needle) === true;
                            } else if (field === 'TeleportPartner') {
                                const hay = UtilHelper.toNormalForm((item[field] as ITP)?.Title?.toString()?.toLowerCase());
                                return hay?.includes(needle) === true;
                            } else {
                        ...
                        if (filter.name === 'TeleportPartner') {
                            return String(item[field]?.Id) === String((f as IDropdownOption).key);
                        }
                        return true;
                    });
    ...
    private sort = (items) => {
        ...
        const key = sort.Property as keyof ISNP;
    ...
    private renderItemColumn = (item, index: number, column: IColumn) => {
        ...
        const fieldName = column.fieldName as keyof ISNP | undefined;
        ...
        if (fieldName === 'Country') {
            const countryListType = ViewType.Country;
        ...
        if (fieldName === 'TeleportPartner') {
            const teleportPartnerListType = ViewType.TeleportPartner;
            return (
                <div className={styles.termPills}>
    ...
    private onChangeTeleportPartner = (options: IDropdownOption[]) => {
        let { filters } = this.state;
        const teleportFilter = filters.find((f) => f.name === 'TeleportPartner');
        teleportFilter.selectedValues = options.length > 1 ? [options[options.length - 1]] : options;
        filters = filters.map(((f) => ((f.name === teleportFilter.name) ? teleportFilter : f)));
        this.setState({ filters, pageIndex: 0 });
    };
    ...
    public render(): React.ReactElement<ISNPViewProps> {
        const { filters, columns, searchKeyword, pageIndex, pageSize, isFilterPanelOpen } = this.state;
        const countryFilter = filters.find((f) => f.name === 'Country');
        const contentTypeFilter = filters.find((f) => f.name === 'ContentType');
        const teleporPartnerFilter = filters.find((f) => f.name === 'TeleportPartner');
                                ...
                                <>
                                    ...
                                    >
                                        <div className={styles.containerPaneldiv}>
                                            ...
                                            {/* TeleportPartner */}
                                            {
                                                this.isFilterEnabled('TeleportPartner') &&
                                                <>
                                                    <div >
                                                        <Label className={styles.filterLabel}>{teleporPartnerFilter.displayName}</Label>
                                                    </div>
                                                    <div className={styles.panelChildDiv}>
                                                        <MultiselectWrapper
                                                            placeholder={teleporPartnerFilter.placeHolder}
                                                            data={teleporPartnerFilter?.values as IDropdownOption[]}
                                                            dataKey={(item: IDropdownOption) => item.key}
                                                            textField={(item: IDropdownOption) => item.text}
                                                            value={teleporPartnerFilter?.selectedValues}
                                                            filter="contains"
                                                            showSelectedItemsInList
                                                            showPlaceholderWithValues
                                                            renderTagValue={() => null}
