...
$DOCUMENT_SPACE = @{
    ...
    TELEPORT_PARTNER_FOLDER  = "/Teleport Partners"
    HYPERLINK_DESC           = "Link"
}
...
# ================================
$CONTENT_TYPE = "0x0100FAB3C8B22A402D4296663A098E9A9FF5"
...
try {
    Write-Log "================================"
    Write-Log "Teleport Partner List Data Import Started"
    Write-Log "================================"
    ...
    $marketLookupMap = Get-LookupMap -ListName $SP_LISTS.MARKET -FieldName "Title"
    ...
    foreach ($record in $csvData) {
        ...
        try {
            Write-Log "Processing record $recordCount/$($csvData.Count): Teleport Partner=$($record.($CSV_COLUMNS.TITLE))"
            ...
            $marketRawValue = $record.($CSV_COLUMNS.MARKET).Trim()
            $marketValues = $marketRawValue -split '[,;|]' |
                ForEach-Object { $_.Trim() } |
                Where-Object { -not [string]::IsNullOrWhiteSpace($_) } |
                Select-Object -Unique

            if (-not $marketValues -or $marketValues.Count -eq 0) {
                Add-RowSkip -RowNumber $recordCount -Reason "Skipped - Market field is empty"
                continue
            }
            ...
            foreach ($marketValue in $marketValues) {
                $marketKey = $marketValue.ToLowerInvariant()
                if ($marketLookupMap.ContainsKey($marketKey)) {
                    $marketIds += [int]$marketLookupMap[$marketKey]
                }
                else {
                    $missingMarkets += $marketValue
                }
            }

            if ($missingMarkets.Count -gt 0) {
                Add-RowSkip -RowNumber $recordCount -Reason "Skipped - Market lookup not found: '$($missingMarkets -join ", ")'"
                continue
            }
            ...
            if ($marketIds.Count -gt 0) {
                ...
                Add-RowSkip -RowNumber $recordCount -Reason "Skipped - Market lookup resolution returned no IDs"
                continue
            }
            ...
            if ($batch.Count -ge $BatchSize -or $recordCount -eq $csvData.Count) {
                ...
                foreach ($batchItem in $batch) {
                    try {
                        if ($DryRun) {
                            Write-Log "DRY RUN: Would add item for Teleport Partner: $($batchItem.Data.Title)"
                        }
                        else {
                            $newItem = $null
                            $retryCount = 0
                            ...
                            try {
                                ...
                                $parentFolderUrl = "$($DOCUMENT_SPACE.LIBRARY_NAME)$($DOCUMENT_SPACE.TELEPORT_PARTNER_FOLDER)"
                            ...
                            Write-Log "✅ Successfully added item ID $itemId for Teleport Partner: $($batchItem.Data.Title)" "SUCCESS"
                            $script:ImportedCount++
                        }
                    }
                    catch {
                        Add-RowError -RowNumber $batchItem.Index -Reason "Error adding item for Teleport Partner '$($batchItem.Data.Title)': $($_.Exception.Message)"
                    }
                }
