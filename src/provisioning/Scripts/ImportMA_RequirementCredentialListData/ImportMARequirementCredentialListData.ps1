...
$SP_FIELDS = @{
    ...
    CREDENTIAL_TYPE          = "MA_CredentialType"
    SUMMARY                  = "MA_Summary"
    MARKET                   = "MA_Market"
    ...
    APPLICATION_DATE         = "MA_RequirementApplicationDate"
    ESTIMATED_DATE           = "MA_RequirementEstimatedDate"
    TRIAL_DEMO               = "MA_IsTrial"
    RENEWAL_TERM             = "MA_RenewalTerm"
    EFFECTIVE_DATE           = "MA_RequirementEffectiveDate"
    EXPIRATION_DATE          = "MA_RequirementExpirationDate"
...
$DOCUMENT_SPACE = @{
    ...
    CREDENTIAL_FOLDER = "/MA Requirements/Credential"
    HYPERLINK_DESC    = "Link"
}
...
# ================================
$CONTENT_TYPES = @{
    "Credential" = "0x01009E58F9850C7D4144AD7FB9331AD6A24802"
}
...
# ================================
$FOLDER_URL_MAP = @{
    "Credential" = $DOCUMENT_SPACE.CREDENTIAL_FOLDER
}
...
function Set-BoolFieldIfProvided {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$ItemData,
    ...
    $value = Get-BoolValue -RawValue $RawValue
    if ($null -eq $value) {
        Add-RowSkip -RowNumber $RowNumber -Reason "Skipped - Invalid $FieldLabel value: '$RawValue'. Allowed values: Yes/No"
        return $false
    }

    $ItemData[$FieldInternalName] = [bool]$value
    return $true
}

function Set-HyperlinkFieldIfProvided {
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$ItemData,
    ...
    $trimmed = $Url.Trim()
    if (-not [Uri]::IsWellFormedUriString($trimmed, [UriKind]::Absolute)) {
        Add-RowSkip -RowNumber $RowNumber -Reason "Skipped - Invalid $FieldLabel URL: '$trimmed'"
        return $false
    }

    $ItemData[$FieldInternalName] = "$trimmed, $Description"
    return $true
}
...
function Convert-ToUrlPath {
    ...
    $segments = $RawPath.Trim('/').Split('/') | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }
...
function Find-RecurrencePatternLookupId {
    ...
    $normalizedTitle = $RequirementTitle.Trim().ToLowerInvariant()
    ...
    #     Write-Log "Recurrence Pattern exact match not found for '$RequirementTitle'. Using contains match '$matchedKey'." "WARN"
...
function Update-DocumentSpaceLink {
    ...
    try {
        $parentFolderPath = $FOLDER_URL_MAP["Credential"]
        if ([string]::IsNullOrWhiteSpace($parentFolderPath)) {
            $parentFolderPath = $DOCUMENT_SPACE.CREDENTIAL_FOLDER
        }
...
try {
    Write-Log "================================"
    Write-Log "MA Requirement Credential List Data Import Started"
    Write-Log "================================"
    ...
    foreach ($record in $csvData) {
        ...
        try {
            ...
            $credentialTypeRaw = [string]$record.($CSV_COLUMNS.CREDENTIAL_TYPE)
            ...
            $credentialTypeRaw = if (-not [string]::IsNullOrWhiteSpace($credentialTypeRaw)) { $credentialTypeRaw.Trim() } else { "" }
            ...
            $credentialTypeMapped = Get-MappedChoiceValue -RawValue $credentialTypeRaw -Map $CREDENTIAL_TYPE_MAPPING
            if ([string]::IsNullOrWhiteSpace($credentialTypeMapped)) {
                Add-RowSkip -RowNumber $recordCount -Reason "Skipped - Invalid Credential Type value: '$credentialTypeRaw'"
                continue
            }
            ...
            $itemData = @{
                ...
                $SP_FIELDS.CONTENTTYPE_ID = $CONTENT_TYPES["Credential"]
                $SP_FIELDS.CREDENTIAL_TYPE = $credentialTypeMapped
            ...
            if (-not (Set-ChoiceFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.RENEWAL_TERM -RawValue ([string]$record.($CSV_COLUMNS.RENEWAL_TERM)) -Map $RENEWAL_TERM_MAPPING -FieldLabel "Renewal Term" -RowNumber $recordCount)) { continue }

            # For recurrent renewal terms, recurrence pattern is mandatory.
            if ($itemData.ContainsKey($SP_FIELDS.RENEWAL_TERM) -and [string]$itemData[$SP_FIELDS.RENEWAL_TERM] -eq "Recurrent") {
                $recurrencePatternId = Find-RecurrencePatternLookupId -RequirementTitle $titleValue -RecurrenceLookupMap $recurrencePatternLookupMap
                if ($null -ne $recurrencePatternId) {
                    $itemData[$SP_FIELDS.RECURRENCE_PATTERN] = [int]$recurrencePatternId
                }
                else {
                    Add-RowSkip -RowNumber $recordCount -Reason "Skipped - Recurrence Pattern not found for title '$titleValue' while Renewal Term is 'Recurrent'"
                    continue
                }
            }
            ...
            if (-not (Set-BoolFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.TRIAL_DEMO -RawValue ([string]$record.($CSV_COLUMNS.TRIAL_DEMO)) -FieldLabel "Trial / Demo" -RowNumber $recordCount)) { continue }

            if (-not (Set-DateFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.APPLICATION_DATE -RawValue ([string]$record.($CSV_COLUMNS.APPLICATION_DATE)) -FieldLabel "Application Date" -RowNumber $recordCount)) { continue }
            if (-not (Set-DateFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.ESTIMATED_DATE -RawValue ([string]$record.($CSV_COLUMNS.ESTIMATED_DATE)) -FieldLabel "Estimated Date" -RowNumber $recordCount)) { continue }
            if (-not (Set-DateFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.EFFECTIVE_DATE -RawValue ([string]$record.($CSV_COLUMNS.EFFECTIVE_DATE)) -FieldLabel "Effective Date" -RowNumber $recordCount)) { continue }
            if (-not (Set-DateFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.EXPIRATION_DATE -RawValue ([string]$record.($CSV_COLUMNS.EXPIRATION_DATE)) -FieldLabel "Expiration Date" -RowNumber $recordCount)) { continue }
            ...
            Set-CommentFieldIfProvided -ItemData $itemData -FieldInternalName $SP_FIELDS.REFERENCE -RawValue ([string]$record.($CSV_COLUMNS.REFERENCE))
            ...
            if ($batch.Count -ge $BatchSize -or $recordCount -eq $csvData.Count) {
                ...
                foreach ($batchItem in $batch) {
                    try {
                        if ($DryRun) {
                            Write-Log "DRY RUN: Would create MA Requirement item for '$($batchItem.Title)'"
