...
$DOCUMENT_SPACE = @{
    ...
    DISTRIBUTION_PARTNER_FOLDER  = "/Distribution Partners"
    HYPERLINK_DESC               = "Link"
}
...
# ================================
$CONTENT_TYPE = "0x010025FCA5695FFF2D4CA3BD04B70D983932"
...
try {
    Write-Log "================================"
    Write-Log "Distribution Partner List Data Import Started"
    Write-Log "================================"
    ...
    foreach ($record in $csvData) {
        ...
        try {
            Write-Log "Processing record $recordCount/$($csvData.Count): Distribution Partner=$($record.($CSV_COLUMNS.TITLE))"
            ...
            if ($batch.Count -ge $BatchSize -or $recordCount -eq $csvData.Count) {
                ...
                foreach ($batchItem in $batch) {
                    try {
                        if ($DryRun) {
                            Write-Log "DRY RUN: Would add item for Distribution Partner: $($batchItem.Data.Title)"
                        }
                        else {
                            $newItem = $null
                            $retryCount = 0
                            ...
                            try {
                                ...
                                $parentFolderUrl = "$($DOCUMENT_SPACE.LIBRARY_NAME)$($DOCUMENT_SPACE.DISTRIBUTION_PARTNER_FOLDER)"
                            ...
                            Write-Log "✅ Successfully added item ID $itemId for Distribution Partner: $($batchItem.Data.Title)" "SUCCESS"
                            $script:ImportedCount++
                        }
                    }
                    catch {
                        Add-RowError -RowNumber $batchItem.Index -Reason "Error adding item for Distribution Partner '$($batchItem.Data.Title)': $($_.Exception.Message)"
                    }
                }
